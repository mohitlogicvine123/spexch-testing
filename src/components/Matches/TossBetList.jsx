      ...
      <div className="md:p-4 p-3">
        ...
        {location?.pathname?.includes('/toss-revert-bets') ? 
          <div className="w-full flex justify-end" onClick={()=>setRemarkModal2(true)}>
            <button disabled={checkbox?.length == 0 ? true : false} className="bg-gradient-seablue disabled:bg-sky-300 py-2 max-w-32 px-3 rounded-md text-white w-full font-semibold text-sm">
              Revert Bets
              </button>
          </div>
          :
          <div className="w-full flex justify-end" onClick={()=>setRemarkModal(true)}>
            <button disabled={checkbox?.length == 0 ? true : false} className="bg-red-500 disabled:bg-red-200 py-2 max-w-32 px-3 rounded-md text-white w-full font-semibold text-sm">
              Delete Bets
              </button>
          </div>}
